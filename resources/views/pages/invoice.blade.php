    ...
    <section>
        <div class="lg:mx-12 py-5">
            <div class="container mx-auto px-4 pb-4">
                @if (session('userData'))
                    <div class="md:w-1/2 my-4 mx-auto bg-invoice border-red-500 rounded-b text-teal-900 px-4 py-3 shadow-md"
                        role="alert">
                        <div class="flex">
                            <div class="py-1">
                ...
                <hr class="border border-primary-one bg-pri w-full h-[0.35rem] my-3 mx-auto" style="background:#9c27b0;">
            </div>
            <div class="mx-auto">
                <div class=" px-4 space-y-4 ">
                    <div class="border rounded-2xl max-w-sm mx-auto bg-white"
                        style="box-shadow: 0 0 6px 0px rgb(3 32 76);">
                        <h4
                        ...
                        <ul class="my-2 space-y-1 ">
                            ...
                            <li class="space-x-3 px-6">
                                <span>Email:</span> 
                                <strong class="float-right">{{ $order->email ?? '-' }} </strong>
                            </li>
                            <li class="space-x-3 px-6">
                                <span>Paper Type:</span> 
                                <strong class="float-right"> {{ $order->paper_type ?? '-' }} </strong>
                            </li>
                            <li class="space-x-3 px-6">
                                <span>Paper Topic:</span> 
                                <strong class="float-right">{{ $order->paper_topic ?? '-' }} </strong>
                            </li>
                            <li class="space-x-3 px-6">
                                <span>Subject:</span> 
                                <strong class="float-right">{{ $order->subject ?? '-' }} </strong>
                            </li>
                            <li class="space-x-3 px-6">
                                <span> Academic Level:</span> 
                                <strong class="float-right">{{ $order->academic_level ?? '-' }}</strong>
                            </li>
                            <li class="space-x-3 px-6">
                                <span>deadline:</span> 
                                <strong class="float-right">{{ $order->deadline ?? '-' }}</strong>
                            </li>
                            <hr class="border-[2px]">
                            <li class="space-x-3 text-xl text-primary-two py-3 px-6">
                                <strong>Total Amount:</strong>
                                <strong class="float-right ">{{ addCurrency($invoice->amount) }}</strong>
                            </li>
                        </ul>
                        ...
                        <div class="w-full px-6 py-4">
                            <a href="{{ url('https://checkout.cheapresumewriter.com/payment?reference=' . $invoice->ref_no . '&gateway=stripe&source=m-ahuk') }}"
